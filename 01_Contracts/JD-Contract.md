...
**MERGED IMPLEMENTATION:** migration `000002_interview_blueprints.up.sql` drops `job_descriptions.blueprint` and creates `interview_blueprints(id, job_description_id, difficulty, duration_minutes, question_count, blueprint_data, contract_version, timestamps)`, with `job_description_id ... ON DELETE RESTRICT`.
