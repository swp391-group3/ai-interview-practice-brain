...
| **Auth** | Stateless JWT authentication with access/refresh tokens in `pkg/token` | `ACCEPTED` | `api/pkg/token/token.go`, `api/internal/features/auth/` | 2026-09-18 | - | Merged access middleware and refresh-cookie boundary; see [[API-Contract]]. |
...
| **JD Domain** | Formal JSON schema for Interview Blueprint | `JIRA DONE / IMPLEMENTATION GAP` | [[KAN-46]], [[JD-Blueprint]] | 2026-09-18 | - | Contract work done; full target DB/session realization is not established. |
