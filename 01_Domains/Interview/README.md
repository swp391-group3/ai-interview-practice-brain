...
Orchestrate a lifelike, conversational technical interview simulation. Executes assessment plans defined by internal blueprints, dynamically probes candidate answers, synchronizes spoken audio with 3D avatar animations, and captures rich conversation turns for evaluation.
...
* **Interview Configuration (Composite Capability):**
  ...
  * **3D Interviewer Persona:** Visual avatar appearance choice for the 3D interviewer.
  * **Voice Profile:** Synthesized voice profile (tone, accent, gender) sourced from TTS providers.
  * **Interview Environment:** Virtual 3D room/background choice.
  * **Difficulty Level:** `easy`, `medium`, or `hard` (independent of role seniority).
  * **Duration & Question Budget:** Planned time limit and question budget.
...
* **Candidate:** Configures the composite interview parameters; completes preflight hardware checks; creates, joins, pauses, resumes, and concludes the live spoken interview.
...
stateDiagram-v2
    ...
    PREFLIGHT --> READY: Mic & audio readiness verified
...
1. **Question Delivery:** The system selects or generates the next question according to the internal blueprint's active competency slot. The TTS engine synthesizes audio along with viseme blend-shape timing frames.
2. **Avatar Spoken Articulation:** The browser plays the audio stream while the 3D virtual interviewer articulates in synchronization using blend-shape visemes.
3. **Candidate Response:** The candidate speaks into their microphone. Voice Activity Detection (VAD) monitors speech boundaries.
4. **Speech-to-Text (STT):** Audio streams to the STT provider, producing an accurate text transcript.
...
3. **Composite Configuration Step:**
   Interviewer selection, voice profile selection, 3D environment selection, difficulty, and duration are configured as part of a single composite `Configure Interview Session` action.
...
* **LLM Provider:** Generates internal blueprint structures, orchestrates conversational dialogue, and devises dynamic follow-up questions.
